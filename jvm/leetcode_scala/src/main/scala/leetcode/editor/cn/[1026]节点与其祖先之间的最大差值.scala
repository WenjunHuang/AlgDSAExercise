...
//leetcode submit region begin(Prohibit modification and deletion)
  ...
  object Solution {
    ...
    private case class Info(min: Int, max: Int)
    private def getMinMax(node: TreeNode): Int =
      var res = 0
      def impl(node: TreeNode): Info =
        node match
          case null => Info(Int.MaxValue, Int.MinValue)
          case _ =>
            val left    = impl(node.left)
            val right   = impl(node.right)
            val rootMin = math.min(node.value, math.min(left.min, right.min))
            val rootMax = math.max(node.value, math.max(left.max, right.max))
            res = math.max(res, math.max(rootMax - node.value, node.value - rootMin))
            Info(rootMin, rootMax)
      impl(node)
      res
