...
//leetcode submit region begin(Prohibit modification and deletion)
  /** Definition for a binary tree node. class TreeNode(_value: Int = 0, _left: TreeNode = null, _right: TreeNode = null) { var value: Int = _value var left: TreeNode = _left var right: TreeNode =
    ...
    */
  object Solution {
    private var res = Integer.MIN_VALUE
    def maxPathSum(root: TreeNode): Int =
      res = Integer.MIN_VALUE
      impl(root)
      res

    private def impl(node: TreeNode): Int =
      if node == null then 0
      else
        val leftMax    = math.max(0, impl(node.left))
        val rightMax   = math.max(0, impl(node.right))
        val pathMaxSum = node.value + leftMax + rightMax
