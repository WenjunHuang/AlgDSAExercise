...
//leetcode submit region begin(Prohibit modification and deletion)
  ...
  object Solution {
    ...
    private def traverse(node: TreeNode, targetSum: Int): Boolean =
      node match
        case null => false
        case _ =>
          if node.left == null && node.right == null then targetSum == node.value
          else traverse(node.left, targetSum - node.value) || traverse(node.right, targetSum - node.value)
  }
