...
//leetcode submit region begin(Prohibit modification and deletion)
  ...
  object Solution {
    def countUnivalSubtrees(root: TreeNode): Int =
      if root == null then 0
      else getUnivalue(root)

    private def getUnivalue(node: TreeNode): Int =
      var res = 0

      def impl(node: TreeNode): Int =
        val left  = if node.left == null then node.value else impl(node.left)
        val right = if node.right == null then node.value else impl(node.right)

        if left == -1001 || right == -1001 then -1001
        else if left == right && node.value == left then
          res += 1
          node.value
        else -1001
