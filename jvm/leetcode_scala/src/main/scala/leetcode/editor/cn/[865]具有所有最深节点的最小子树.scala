...
//leetcode submit region begin(Prohibit modification and deletion)
  ...
  object Solution {
    ...
    case class Result(node: TreeNode, depth: Int)
    def maxDepth(node: TreeNode): Result =
      node match
        case null => Result(null, 0)
        case _ =>
          val left  = maxDepth(node.left)
          val right = maxDepth(node.right)
          if left.depth == right.depth then Result(node, left.depth + 1)
          else
            val res = if left.depth > right.depth then left else right
            res.copy(depth = res.depth + 1)
  }
