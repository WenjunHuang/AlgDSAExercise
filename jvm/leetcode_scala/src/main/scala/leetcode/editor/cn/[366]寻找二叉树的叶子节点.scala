...
//leetcode submit region begin(Prohibit modification and deletion)
  ...
  object Solution {
    ...
    private def impl(node: TreeNode): (TreeNode, List[Int]) =
      if node == null then (null, Nil)
      else if node.left == null && node.right == null then (null, List(node.value))
      else
        val (removeLeft, left)   = impl(node.left)
        val (removeRight, right) = impl(node.right)
