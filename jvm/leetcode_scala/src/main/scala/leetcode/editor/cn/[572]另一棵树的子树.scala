...
//leetcode submit region begin(Prohibit modification and deletion)
  ...
  object Solution {
    ...
    def isSubtree(root: TreeNode, subRoot: TreeNode): Boolean =
      impl(root, subRoot).result

    private def impl(node: TreeNode, subRoot: TreeNode): TailRec[Boolean] =
      if node == null && subRoot == null then done(true)
      else if node == null || subRoot == null then done(false)
      else
        isSame(node, subRoot).flatMap { r =>
          if r then done(true)
          else
            impl(node.left, subRoot).flatMap { r =>
              if !r then impl(node.right, subRoot)
              else done(true)
            }
        }

    private def isSame(one: TreeNode, two: TreeNode): TailRec[Boolean] =
      if one == null && two == null then done(true)
      else if one == null || two == null then done(false)
      else if one.value != two.value then done(false)
      else
        isSame(one.left, two.left).flatMap { r =>
          if r then isSame(one.right, two.right)
          else done(false)
        }
  }
