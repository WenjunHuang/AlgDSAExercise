...
//leetcode submit region begin(Prohibit modification and deletion)
  ...
  object Solution {
    def longestUnivaluePath(root: TreeNode): Int =
      root match
        case null => 0
        case _    => maxLen(root, root.value)

    private def maxLen(node: TreeNode, parentVal: Int): Int =
      var res = 0
      def impl(node: TreeNode, parentVal: Int): Int =
        node match
          case null => 0
          case _ =>
            val leftLen  = impl(node.left, node.value)
            val rightLen = impl(node.right, node.value)
            res = math.max(res, leftLen + rightLen)
            if node.value != parentVal then 0
            else math.max(leftLen, rightLen) + 1
      impl(node, parentVal)
      res
  }
