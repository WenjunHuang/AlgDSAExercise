...
//leetcode submit region begin(Prohibit modification and deletion)
  ...
  object Solution {
    ...
    def isSymmetric(root: TreeNode): Boolean = root match
      case null => true
      case _    => impl(root.left, root.right).result

    private def impl(one: TreeNode, two: TreeNode): TailRec[Boolean] =
      (one, two) match
        case (null, null)                => done(true)
        case (null, _)                   => done(false)
        case (_, null)                   => done(false)
        case _ if one.value != two.value => done(false)
        case _ =>
          impl(one.left, two.right).flatMap { r =>
            if r then impl(one.right, two.left)
            else done(false)
          }
  }
